<x-layout.site :description="$seo?->team_meta_description">
  <div class="h-full md:px-32">
    <x-grid.container class="h-full">
      <x-grid.span class="md:col-span-8 md:-ml-32 md:min-h-0">
        @if($page?->media->count())
          <x-media.image :media="$page->media" sizes="(min-width: 768px) 66vw, 100vw" class="w-full h-full object-cover" />
        @endif
      </x-grid.span>
      ...
      <x-grid.span class="col-span-4 px-16 py-16 md:px-0 min-h-0 overflow-auto">
        @foreach($members as $member)
          <div @class(['mb-32' => !$loop->last])>
            <h2 class="text-[23px] leading-[1.18] mb-0">{{ $member->firstname }} {{ $member->name }}</h2>
