...
<nav>
  <ul class="flex items-center gap-x-60">
    <li class="flex items-center gap-x-52 order-2 md:order-1">
      @if($prev)
        <a href="{{ route('page.project.images', $prev->slug) }}" aria-label="Vorheriges Projekt" class="block w-15 h-27 text-black">
          <x-icons.chevron-left />
        </a>
      @endif
      @if($next)
        <a href="{{ route('page.project.images', $next->slug) }}" aria-label="Nächstes Projekt" class="block w-15 h-27 text-black">
